...
labels <- c( "CLAW CFL0.95 un", "FARSIGHT CFL1.0", "SLFV-ML CFL0.8 un", "UCISOM CFL1.0", 
            "CAMFV CFL1.2", "Pangolin 0.65x0.43° CFL0.95", "Pangolin 0.14x0.09° CFL0.95")
...
p <- p + ggtitle("Filament diagnostics")
# Customize
...
# Legends and color
# We need to set both scale to merge the legends
p <- p + scale_color_manual( name  ="Models", labels = labels, 
   values=cbPalette)
p <- p + scale_shape_manual( name  ="Models", labels = labels, 
   values=seq(1,17))

p <- p + scale_linetype_discrete( guide=FALSE)
# Legend in figure, bottom left
p <- p + theme(legend.justification=c(0,0), legend.position=c(0,0))

ggsave("filaments_compare.pdf", width=10, height=8)
#dev.off()
