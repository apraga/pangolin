...
min_chem_rac <- seq(length(nb_procs))

cur <- paste(dir, "/profiling_3", sep="")
time_seq <- read.table(cur)
# 90 latitude on an hemisphere
time_seq_chem <- 2*90**2*chem_time
time_seq_chem_rac <- 2*90**2*chem_time_rac
...
for (n in nb_procs) {
    ...
    time_chem_optim <- nb_cells*chem_time_rac
    ...
    min_chem_rac[i] <- min((tmp_seq + time_seq_chem_rac) / (time2[6]))
    i <- i+1
}
...
plot_all <- function() {
  ...
  color <- c("red", "blue", "seagreen")
  style <- c("dashed", "dotted", "solid")
  # Advection
  args <- list(nb_procs, min, ylim=extrem, ylab="",xlab="", 
  ...
  legend_pos = "topleft"
  legend(legend_pos, c("advection only", "chemistry(MOCAGE)",
  "chemistry (RACMOBUS+ASIS)"), lty=style, lwd=2, col=color)
}


# Avoid to go outside A4 size...
if (savefig) 
  postscript(titlefig,width=8,height=8, pointsize=15)
par(lwd=2)
plot(nb_procs, y, type='l', ylim=extrem, ylab="speedup min", xlab="cores")
par(new=TRUE)
plot_all()
if (savefig) dev.off()
