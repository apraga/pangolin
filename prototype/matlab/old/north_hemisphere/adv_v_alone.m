...
%
function [C_s,f_v,mC_un_s] = adv_v_alone(C_l,v_s,mco,dco,mcoo,dcoo,nindp,nindm,nlat,nlon,nsec,dt)
  %
  %
  % Calcul des flux
  %
  ...
  for i=1:nlat;
    ...
    for j=1:jl;
      ind = 3*(i-1)*(i-1) + j;
      mC_s(i,j) = C_l(ind)*dlat*dlon;
      mC_un_s(i,j) = dlat*dlon;
    end;
  end;
