...
for i in range(nb_lat):
  ...
  for j in range(nb_mesh):
    ...
    i_prev = int((89.5-lat_prev)/dlat)
    j_prev = int(lon_prev/dlon-0.5)
    k_prev = int(i_prev*nb_lon + j_prev)

#    if (k_prev > nb_points-1):
#      print i_prev,j_prev,i,j,k_prev
    if (k > nb_points-1):
      print "indices",i_prev,j_prev,i,j,k
      print "lon, lat",lat_prev,lon_prev
    lat[k] = lat_cur
    lon[k] = lon_cur
    if (lon_cur > 180):
      lon[k] = lon_cur - 360
...
name_fig = 'bidim_anal_'+str(nb_iter)+'.eps'
title = "Solution analytique (projection de Miller)" 
p.title(title)
p.savefig(name_fig)
