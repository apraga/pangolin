...
###############################################################################
# Read a file on format (val1,val2,val3) and return them
# It is faster to operate on an array
###############################################################################
def read_data(filename):
  nb_lines = lib_plot.file_len(filename)
  f = open(filename, 'r')
...
# Error with 2-norm
################################################################################
def error_2(q_anal,q_num,lat,lon):
  num = (q_num - q_anal)**2
  denom = q_anal**2
...
# Error with max norm
################################################################################
def error_oo(q_anal,q_num,lat,lon):
  diff = abs(q_num - q_anal)
  q_abs = abs(q_anal)
...
################################################################################
def error_quad(q_anal,q_num,lat,lon):
  # Residual sum of squares
  square = (q_num - q_anal)**2
  res = sum(square)
  # Divided by the number of degree of freedom
  return res/len(q_anal)

################################################################################
# Wrapper
################################################################################
def get_error(anal,num,error):
  lat,lon,q_anal = read_data(anal)
  lat1,lon1,q_num = read_data(num)
  
  #check_grids(lat,lon,lat1,lon1)
  
  if (error == '2'):
    res = error_2(q_anal,q_num,lat,lon)
  elif (error == 'oo'):
    res = error_oo(q_anal,q_num,lat,lon)
  elif (error == 'quad'):
    res = error_quad(q_anal,q_num,lat,lon)
  else:
